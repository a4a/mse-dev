...
\centering
<<fig=TRUE, echo=FALSE>>=
xyplot(data~year|par, groups=ref, data=subset(base.summ, ref %in% c("base", "srvBias0.5", "srvBias0.75") & qtl==0.5), scales=list(y=list(relation="free")), par.settings=list(superpose.line=list(col=c("grey40", "red", "green"))), type="l", par.strip.text=list(cex=0.5), ylab="year", auto.key=list(lines=TRUE, points=FALSE, columns=3))
@
\caption{Survey coverage effect, bias=0.5, 0.75+higher variability on mean}
\label{fig:srvBias2}
\end{figure}
 <- lh(gislasim(par), range=c(min=1, max=8, minfbar=1, maxfbar=6))
