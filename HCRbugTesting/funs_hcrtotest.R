...
a4aDefaults <- function(stock, indices) {
  ...
  # FMODEL
  fmodel <- list(
    # separable ~ s(age, k = 2/3 * ages) + s(year, k = 2/3 * years)
    separable = list("~ s(age, k = KA) + s(year, k = KY)",
      KA=floor(2/3 *dis$age), KY=floor(2/3 * dis$year)),

    # interaction ~ te(age, year, k=c(2/3 * ages, 2/3 * years))
    interaction = list("~ te(age, year, k=c(KA, KY))",
      KA=floor(2/3 *dis$age), KY=floor(2/3 * dis$year))
  )

  # QMODEL
  qmodel <- list(
    # constant ~ s(age, k = 2/3 * ages) by index
    constant = lapply(lds, function(x)
      list("~s(age, k=KA)", KA=floor(2/3 * x$age))),
    # linear ~ s(age, k = 2/3 * ages) + year by index
    linear = lapply(lds, function(x)
        list("~s(age, k=KA) + year", KA=floor(2/3 * x$age)))
  )

  # SRMODEL
  srmodel <- list(
    base = list("~ factor(year)"),
    bevholt = list("~bevholt(CV=0.2)"),
    ricker = list("~ricker(CV=0.2)"),
    bevholtSV = list("~bevholtSV(CV=0.2)"),
    hockey = list("~hockey(CV=0.2)"),
    geomean = list("~geomean(CV=0.2)")
  )

  # VMODEL
  vmodel <- list(
    # base ~ s(age, k = 1/2 ages) for catch, ~1 for indices
    base = c(list(list("~s(age, k=KA)", KA=floor(1/2 * dis$age))),
        lapply(lds, function(x) "~1"))
  )

  # N1MODEL
  n1model <- list(
    # base ~ factor(age)
    base = list("~ factor(age)"),
    # base ~ s(age, k=ages)
    limited = list("~s(age, k = KA)", KA=dis$age)
  )

  return(list(fmodel=fmodel, qmodel=qmodel, vmodel=vmodel,
    srmodel=srmodel, n1model=n1model))
} # }}}
...
getF <- function(trgt, fc, mxy, ay, object, multiplicative=TRUE, correction=TRUE){
    ...
    if(multiplicative){
        ry1 <- 1-(trgt/object[,ac(ay)])^(1/(mxy-ay))
